...
/*
	...
	dAnalysisActions.push_back(new DCutAction_PIDDeltaT(dComboWrapper, false, 0.8, KMinus, SYS_BCAL));
	// dAnalysisActions.push_back(new DCutAction_PIDDeltaT(dComboWrapper, false, 1, KMinus, SYS_FCAL));
	dAnalysisActions.push_back(new DCutAction_PIDDeltaT(dComboWrapper, false, 0.35, KMinus, SYS_TOF));
	...
	//CUT MISSING MASS
	dAnalysisActions.push_back(new DHistogramAction_MissingMassSquared(dComboWrapper, false, 600, -0.02, 0.02, "mm2_meas"));
	// dAnalysisActions.push_back(new DHistogramAction_MissingMassSquared(dComboWrapper, true, 600, -0.1, 0.1, "mm2_kinfit"));
	dAnalysisActions.push_back(new DCutAction_MissingMassSquared(dComboWrapper, false, -0.005, 0.005));
*/
	//BEAM ENERGY
	...
	h_PhiMass_Thrown = new TH1F("PhiMass_Thrown", ";m_{K^{+}K^{-}} [GeV/c^{2}];Counts", 600, 0.99, 1.5);
	h_foMass_Thrown = new TH1F("foMass_Thrown", ";m_{#pi^{+}#pi^{-}} [GeV/c^{2}];Counts", 600, 0.28, 1.5);
	h_YMass_Thrown = new TH1F("YMass_Thrown", ";m_{K^{+}K^{-}#pi^{+}#pi^{-}} [GeV/c^{2}];Counts", 600, 1.5, 3.2);
	...
	// Chi2/ndf
	hchi2ndf = new TH1F("hchi2ndf", "#chi^{2}/ndf of the Kinematic Fit (P4 + Vertex) ;#chi^{2}/ndf ;Counts", 100, 0, 1000);
...
Bool_t DSelector_pippimkpkm::Process(Long64_t locEntry)
{
	...
	*/
	...
	for(UInt_t loc_i = 0; loc_i < Get_NumThrown(); ++loc_i)
	{
		...
		if (loc_i == 0) locPiPlusP4_Thrown = locP4_Thrown; // assume order of particles as PID is zero at the moment
		if (loc_i == 1) locPiMinusP4_Thrown = locP4_Thrown;
		if (loc_i == 2) locKPlusP4_Thrown = locP4_Thrown;
		if (loc_i == 3) locKMinusP4_Thrown = locP4_Thrown;
